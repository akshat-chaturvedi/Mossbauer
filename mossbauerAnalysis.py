...
class Analyzer:
    ...
    def spectrumPlotter(self):
        ...
        ax.set_title("Folded Spectrum", fontsize=16)
        ...
        fig, ax = plt.subplots()
        ax.set_xlabel("Velocity [mm s$^{-1}$]", fontsize=14)
        ax.set_ylabel("Counts", fontsize=14)
        ax.set_title(f"Mössbauer Spectrum for {self.compoundName}", fontsize=16)
        ax.scatter(x4, self.folded, s=1)
        ax.set_ylim(np.sort(self.folded)[1]-100, np.sort(self.folded)[-2]+300)
        plt.tick_params(axis='y', which='major', labelsize=14)
        plt.tick_params(axis='x', which='major', labelsize=14)
        fig.savefig(f"{self.name}/{self.name}_calibratedPlot.pdf", dpi=300, bbox_inches="tight")
